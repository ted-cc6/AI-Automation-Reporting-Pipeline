...
# Both RANGE_CHECKS and SCOPE_CHECKS are Africa/Vietnam-schema-specific --
# every column named here is absent from LARCO's mapping (see
# data_loader_larco/column_mapping.csv), which has no equivalent
# type-specific follow-up questions at all. Rather than have Check 2/4 print
# a wall of misleading FAIL rows for columns that were never asked, both
# checks are schema-gated in main() -- LARCO_RANGE_CHECKS/LARCO_SCOPE_CHECKS
# cover only what LARCO's schema actually shares.
RANGE_CHECKS = [
...
LARCO_RANGE_CHECKS = [
    ("q_nps_score",       0,  10, "0–10"),
    ("q_financial_stress", 1,   5, "1–5"),
    ("q_claim_experience_rating", 1, 5, "1–5"),
    ("q_client_age",      18, 100, "18–100"),
]
...
# LARCO has no type-specific follow-up questions at all for any insurance
# type (see data_loader_larco/column_mapping.csv's notes on raw col 8) --
# Check 4 has nothing meaningful to check against this schema.
LARCO_SCOPE_CHECKS: dict[str, dict] = {}
...
def check_2_value_ranges(df: pd.DataFrame, range_checks: list = RANGE_CHECKS) -> tuple[list, list[str]]:
    rows: list = []
    errors: list[str] = []
...
# Each check's (col, base_col_needed_for_its_mask) pair -- base_col is
# checked for presence before the mask is built, since not every schema
# (e.g. LARCO) asks q_insured_event_12m/q_claim_result at all. A check whose
# base column OR target column is absent is reported as "not applicable"
# rather than attempted (previously this indexed df["q_insured_event_12m"]
# etc. unconditionally at the top of the function, which crashed outright on
# a schema missing that column instead of just failing the check). Kept as
# two explicit per-schema lists (rather than one universal "missing =
# N/A" rule) so a genuinely missing column in an Africa/Vietnam run --
# which IS expected to have it -- still fails loudly instead of silently
# becoming N/A.
AFRICA_SKIP_LOGIC_CHECKS = [
    ("SL-1", "q_claim_submitted non-null only where q_insured_event_12m == True",
     "q_claim_submitted", "q_insured_event_12m",
     lambda d: d["q_insured_event_12m"] == True),  # noqa: E712
    ("SL-2", "q_no_claim_reason non-null only where q_claim_submitted == False",
     "q_no_claim_reason", "q_claim_submitted",
     lambda d: d["q_claim_submitted"] == False),  # noqa: E712
    ("SL-3", "q_claim_result non-null only where q_claim_submitted == True",
     "q_claim_result", "q_claim_submitted",
     lambda d: d["q_claim_submitted"] == True),  # noqa: E712
    ("SL-4", "q_claim_challenges_experienced non-null only where q_claim_submitted == True",
     "q_claim_challenges_experienced", "q_claim_submitted",
     lambda d: d["q_claim_submitted"] == True),  # noqa: E712
    ("SL-5", "q_child_improvements non-empty list only where q_child_wellbeing == 'Yes'",
     "q_child_improvements", "q_child_wellbeing",
     lambda d: d["q_child_wellbeing"] == "Yes"),
    ("SL-6", "flag_negative_coping non-null only where q_insured_event_12m == True",
     "flag_negative_coping", "q_insured_event_12m",
     lambda d: d["q_insured_event_12m"] == True),  # noqa: E712
]
# LARCO has no q_insured_event_12m/q_claim_result/q_no_claim_reason/
# q_claim_challenges_experienced/flag_negative_coping equivalents at all
# (see data_loader_larco/column_mapping.csv) -- only the child-improvements
# skip-logic construct (SL-5) has a real LARCO counterpart.
LARCO_SKIP_LOGIC_CHECKS = [
    ("SL-5", "q_child_improvements non-empty list only where q_child_wellbeing == 'Yes'",
     "q_child_improvements", "q_child_wellbeing",
     lambda d: d["q_child_wellbeing"] == "Yes"),
]
...
def check_3_skip_logic(df: pd.DataFrame, checks: list = AFRICA_SKIP_LOGIC_CHECKS
                        ) -> tuple[list, list[str], list[str]]:
    rows: list = []
    errors: list[str] = []
    warnings: list[str] = []

    for check_id, desc, col, base_col, build_mask in checks:
        if base_col not in df.columns:
            rows.append((check_id, desc, "—", "—", "—", "N/A (schema doesn't ask this)"))
            continue
        if col not in df.columns:
...
def check_4_scope(df: pd.DataFrame, scope_checks: dict = SCOPE_CHECKS) -> tuple[list, list[str]]:
    rows: list = []
    errors: list[str] = []
...
def check_5_derived(df: pd.DataFrame, target_country: "str | None" = None,
                     valid_insurance_slugs: frozenset = frozenset({"health", "crop", "credit_life"})
                     ) -> tuple[list, list[str], list[str]]:
    ...
    Every flag block below is gated on the flag column's own presence --
    data_loader_derived.py legitimately skips a flag whose required source
    column(s) aren't in this dataset's schema (e.g. LARCO has no
    q_insured_event_12m, so flag_negative_coping is never computed for it),
    and this check must not treat that as a coding-error crash.
    """
    rows: list = []
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if "flag_negative_coping" not in df.columns:
        rows.append(("flag_negative_coping", "—", "—", "N/A (schema doesn't ask this)"))
    else:
    ...
    if "flag_promoter" not in df.columns:
        rows.append(("flag_promoter", "—", "—", "N/A (schema doesn't ask this)"))
    else:
    ...
    if "flag_paid_claimant" not in df.columns:
        rows.append(("flag_paid_claimant", "—", "—", "N/A (schema doesn't ask this)"))
    else:
    ...
    if "flag_child_wellbeing_denominator" not in df.columns:
        rows.append(("flag_child_wellbeing_denominator", "—", "—", "N/A (schema doesn't ask this)"))
    else:
    ...
    rows.append(("insurance_type — valid slug values only",
                 f"⊆ {sorted(valid_insurance_slugs)}", str(sorted(actual_slugs)),
...
def check_6_fill_rates(df: pd.DataFrame, spec=None, q_definitions: "list | None" = None) -> list:
    """spec: Africa's insurance-report-spec.yaml, used only to filter the
    default Africa q_definitions list down to columns the spec actually
    references. Pass q_definitions explicitly (and leave spec=None) for a
    schema with no such spec document (e.g. LARCO) -- every column presence
    check below is guarded either way, so this never crashes regardless.
    """
    rows: list = []

    insured_mask = df["q_insured_event_12m"] == True if "q_insured_event_12m" in df.columns else None  # noqa: E712
    submitted_mask = df["q_claim_submitted"] == True if "q_claim_submitted" in df.columns else None  # noqa: E712
    health_mask = df["is_health"] == True if "is_health" in df.columns else None  # noqa: E712
    if "q_nps_score" in df.columns:
        ...
        detractor_mask = passive_mask = promoter_mask = None
    ...
    if q_definitions is None:
        ...
        if spec is not None:
            ...
            q_definitions = [d for d in q_definitions if d[0] in seen_refs]
...
def build_report(
    ...
    spec_line = (
        f"- Spec: {len(spec.all_source_questions())} source questions, "
        f"{len(spec.referenced_variables())} metric variables referenced"
        if spec is not None
        else "- Spec: none (no LARCO-equivalent of insurance-report-spec.yaml yet — Check 1 skipped)"
    )
    lines: list[str] = [
...
# LARCO's own Check 6 fill-rate columns -- there is no LARCO equivalent of
# insurance-report-spec.yaml (that document describes the Africa/Vietnam
# instrument specifically), so this is a small hand-picked list rather than
# spec-filtered like Africa's default.
LARCO_Q_DEFINITIONS = [
    ("q_survey_consent",                "all", None, ""),
    ("q_claim_submitted",               "all", None, ""),
    ("q_claim_experience_rating",       "filed a claim", None, "conditional — low fill expected"),
    ("q_vf_services_received",          "all", None, "multi-select list"),
    ("q_product_understanding_combined", "all", None, "trend indicator: product understanding"),
    ("q_financial_stress",              "all", None, ""),
    ("q_quality_of_life_text",          "all", None, "open text"),
    ("q_child_wellbeing",               "all", None, "trend indicator: child wellbeing improvement"),
    ("q_child_improvements",            "all", None, "multi-select list; conditional on child wellbeing improved"),
    ("q_financial_security_belief",     "all", None, ""),
    ("q_alternative_access",            "all", None, "trend indicator: access to alternatives"),
    ("q_prior_access",                  "all", None, "trend indicator: first-time access to insurance"),
    ("q_nps_score",                     "all", None, "trend indicator: client satisfaction"),
    ("q_nps_followup",                  "all", None, ""),
    ("q_sex",                           "all", None, ""),
]
...
def main(output_dir: Path, target_country: "str | None" = None,
    ...
    # Check 1 (spec alignment) has no meaning for LARCO -- insurance-report-
    # spec.yaml documents the Africa/Vietnam instrument specifically, and no
    # LARCO-equivalent spec document exists yet (a real gap, not silently
    # papered over: flagged as a warning in the report rather than a check
    # that would otherwise report ~130 false "column missing" errors).
    if is_larco:
        spec = None
        c1_rows: list = []
        c1_errors: list[str] = []
        c1_warnings = ["Check 1 skipped: no LARCO-equivalent of insurance-report-spec.yaml exists yet."]
        all_warnings.extend(c1_warnings)
        log.info("Check 1: Spec Alignment -- skipped (no LARCO validation spec)")
    else:
    ...
    range_checks = LARCO_RANGE_CHECKS if is_larco else RANGE_CHECKS
    scope_checks = LARCO_SCOPE_CHECKS if is_larco else SCOPE_CHECKS
    valid_insurance_slugs = (
        frozenset({"health", "crop", "credit_life", "personal_accident"}) if is_larco
        else frozenset({"health", "crop", "credit_life"})
    )
    q_definitions = LARCO_Q_DEFINITIONS if is_larco else None
    skip_logic_checks = LARCO_SKIP_LOGIC_CHECKS if is_larco else AFRICA_SKIP_LOGIC_CHECKS

    log.info("Check 2: Value Ranges")
    c2_rows, c2_errors = check_2_value_ranges(df, range_checks=range_checks)
    all_errors.extend(c2_errors)
    _log_check("2", len(c2_errors), 0)

    log.info("Check 3: Skip-Logic Consistency")
    c3_rows, c3_errors, c3_warnings = check_3_skip_logic(df, checks=skip_logic_checks)
    ...
    c4_rows, c4_errors = check_4_scope(df, scope_checks=scope_checks)
    ...
    c6_rows = check_6_fill_rates(df, spec, q_definitions=q_definitions)
